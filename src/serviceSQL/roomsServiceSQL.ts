...
export const getIdRoom =  async (id: number) => {
  return(
    await callDB("SELECT * FROM rooms WHERE id=?", [
      id,
    ])
    )
};
...
export const updateRoom = async (putRoom: Room) =>{
    const resp = await callDB(
        "UPDATE rooms SET photo=?, roomType=?, description=?, roomNumber=?, offer=?, price=?, discount=?, cancellation=?, status=?, amenities=? WHERE id = ?",
        [
            putRoom.photo,
            putRoom.numberRoom,
            putRoom.roomType,
